(function () {
    ...
    TodoClient.prototype.get = function (id, callback) {
        ...
        xhr.open('GET', this.baseUrl + '/api/todo/' + id);
        xhr.onload = function () {
            if (xhr.status === 200) {
    ...
    TodoClient.prototype.getList = function (includeCompleted, includeActive, callback) {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', this.baseUrl + '/api/todo?includecompleted=' + (includeCompleted === true) + '&includeactive=' + (includeActive === true));
        xhr.onload = function () {
            if (xhr.status === 200) {
                callback(null, JSON.parse(xhr.responseText));
            }
    ...
    TodoClient.prototype.create = function (todo, callback) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', this.baseUrl + '/api/todo');
    ...
    TodoClient.prototype.update = function (todo, callback) {
        var xhr = new XMLHttpRequest();
        xhr.open('PUT', this.baseUrl + '/api/todo/' + todo.id);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.onload = function () {
            if (xhr.status === 200) {
                callback(null, JSON.parse(xhr.responseText));
    ...
    TodoClient.prototype.setIsComplete = function (id, isComplete, callback) {
        var xhr = new XMLHttpRequest();
        xhr.open('PATCH', this.baseUrl + '/api/todo/' + id);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.onload = function () {
            if (xhr.status === 200) {
                callback(null, JSON.parse(xhr.responseText));
    ...
    TodoClient.prototype.delete = function (id, callback) {
        var xhr = new XMLHttpRequest();
        xhr.open('DELETE', this.baseUrl + '/api/todo/' + id);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.onload = function () {
            if (xhr.status === 200) {
                callback(null, safeJsonParse(xhr.responseText));
